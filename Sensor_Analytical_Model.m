...
dr  = 1;                             %Radial density
dt  = 5;                             %Thickness density 
a = linspace(300e-6, 300e-6, dr);    %µm, Diaphragm radius
g = 10.0e-6;                         %µm, Gap 10um
t_1 = 1.0e-6;                       %µm, Top Parylene thickness
t_2 = 0.2e-6;                        %µm, Top Electrode thickness
t_3 = linspace(2e-6,10e-6,dt);       %µm, Parylene thickness changing 2-10µm
...
for i = 1:length(a)           %Radial indice
    ...
    for j = 1:length(t_3)       %Thickness indice. For a radius value, sensitivity
        ...
        %% Alican's equation modified
        %{
        % "2" became "1/2" and it is w.r.t top of the diaphragm
        d = -(c_2 * t_1 + c_3 *(t_1 + t_2)) ./ (0.5 .* (c_1 + c_2 + c_3));
        d = abs(d)
        b_1 = h_tot-d;
        b_2 = abs(t(3) - b_1); 
        ...
        b = [b_1 b_2 b_3 b_4];
        % While subtracting b values, abs() is taken

        sum_1 = (E(3) .* abs(b(2)^3 - b(1)^3 )) ./ (1- (v(3).^2));
        sum_2 = (E(2) .* abs(b(3)^3 - b(2)^3 )) ./ (1- (v(2).^2));
        sum_3 = (E(1) .* abs(b(4)^3 - b(3)^3 )) ./ (1- (v(1).^2));
        ...
        if g < w_0
            ...
            PnonT = P(1:indice(1)-1 );
            CapnonT = Capacitance(1:indice(1)-1 );
            [nonTouchModel, ntParameters] = polyfit(PnonT, CapnonT,1);

            Sensitivitynontm = nonTouchModel(1) .* 100;
            Sensitivity1 = cat(2, Sensitivity1, Sensitivitynontm)

            fit1  = polyval(nonTouchModel, PnonT); %fit for nontM

            %Array of data on Touch mode
            P_tM =  PTouchmode;
            Cap_tM = CapacitanceTouchMode;
            [TouchModel,tParameters] = polyfit(P_tM, Cap_tM,1);
    
            Sensitivitytm = TouchModel(1).*100;
            Sensitivity2 = cat(2, Sensitivity2, Sensitivitytm);
    
            fit2 = polyval(TouchModel, P_tM);     %fit for tM
            ...
            %Data on NonTouch mode
             ...
             CapnonT = Capacitance;
             [nonTouchModel, ntParameters] = polyfit(PnonT, CapnonT,1);
             Sensitivitynontm = nonTouchModel(1) .* 100; %Sensitivity fF/100Pa
             Sensitivity1 = cat(2, Sensitivity1, Sensitivitynontm)
             fit1  = polyval(nonTouchModel, PnonT); %fit for nontM
        ...
        %
        %cagri400 = um4um1
        titleCell = {sprintf('R= %f um, t3=  %f um', R*10^6, t(3)*10^6)};
        figure
        plot(P, Capacitance);
        ...
        if g < w_0array(end)
            xline(PTouchmode(1), "Color", "r", "LineStyle","--");
            yline(CapacitanceTouchMode(1), "Color","r","LineStyle","--");
        end
        
        
        %legend("Analytical","TouchMode")
        R2prt = {sprintf('(R_1)^2 = %f', R2nontouch)};
        R2rtmp = {sprintf('(R_2)^2 = %f', R2touch)};

        plot(PnonT, fit1, "--");
        text(P(1),Capacitance(1),R2prt,'right')
        plot(P_tM, fit2, "--");
        %text(P(70),Capacitance(70), R2rtmp,'right')
        legend("Analytical", "Simulation", "Touchmode Pressure", "Touch Mode Capacitance", ...
            "NonTm fit", "Tm fit")
        %}
...
Sensitivitytouch(Sensitivitytouch == 0) = NaN;
...
%{
figure
contourf(a, t_3, transpose(Sensitivitynontouch),30)
xlabel('Radius um');
ylabel('Thickness um');
title('Contour Plot of Sensitivity for nontouch mode (fF/100Pa)');

figure
contourf(a, t_3, transpose(Sensitivitytouch),30)
xlabel('Radius um');
ylabel('Thickness um');
